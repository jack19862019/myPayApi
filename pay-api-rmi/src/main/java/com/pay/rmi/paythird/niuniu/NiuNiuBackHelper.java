...
@Component
public class NiuNiuBackHelper extends CallBackFactory {

    private String flagSuccess = "success";

    public NiuNiuBackHelper init(McpConfigEntity mcpConfig, OrderEntity order, Map<String, String> params) {
    ...
    public NiuNiuBackHelper verifySign() {
        ...
        String newSign = PayMD5.MD5Encode(signStr +"&api_secret="+ mcpConfig.getUpKey());
        Assert.mustBeTrue(newSign.equals(sign), order.getOrderNo() + "订单验签失败！");
        return this;
    }

    public NiuNiuBackHelper checkStatus() {
        String tradeStatus = callBackParams.get("pay_state");
        Assert.mustBeTrue("1".equals(tradeStatus), "支付未成功，终止通知下游！");
        return this;
    }

    public NiuNiuBackHelper updateOrder() {
        String trade_no = callBackParams.get("order_sn");
