package com.pay.rmi.paythird.momozhifu;
...
@Service(Momo.channelNo)
public class Momo extends AbstractPay {

    static final String channelNo = "momofu";

    private static final String payUrl = "http://120.78.65.69:8100/api/sig/v1/";
    ...
    public OrderApiRespParams order(ChannelEntity channel, MerchantEntity merchant, McpConfigEntity mcpConfig, OrderReqParams reqParams) {
        LogByMDC.info(channelNo, "默默支付请求：{}", JSON.toJSONString(reqParams));
        ...
        LogByMDC.info(channelNo, "默默支付响应 订单：{}，response：{}", reqParams.getOrderNo(), result);
        Map<String, String> resultMap = MapToXml.toMap(result);
        String status = resultMap.get("status");
        Assert.isTrue("0".equals(status), "默默支付状态响应:" + resultMap.get("message"));
    ...
    private Map<String, String> getParamsMap(McpConfigEntity mcpConfig, OrderReqParams reqParams) {
        ...
        Assert.notNull(payType, "默默支付不支持的支付方式:" + reqParams.getOutChannel());
        ...
        LogByMDC.info(channelNo, "默默支付参与加签内容：{}", params);
    ...
    public String callback(ChannelEntity channel, MerchantEntity merchant, McpConfigEntity mcpConfig, OrderEntity order, Map<String, String> params) {
        LogByMDC.info(channelNo, "默默支付回调内容：{}", params);
        if (order.getOrderStatus().equals(OrderStatus.succ)) {
            LogByMDC.error(channelNo, "默默支付回调订单：{}，重复回调", order.getOrderNo());
            return "success";
        }
        ...
        try {
            ...
            LogByMDC.info(channelNo, "默默支付回调订单：{}，下发通知成功", order.getOrderNo());
        } catch (Exception e) {
            LogByMDC.error(channelNo, "默默支付回调订单：{}，下发通知失败{}", order.getOrderNo(), e.getMessage());
        }
        return "success";
    }
