package com.pay.rmi.paythird.daxi;
 ...
 * 快银
 */
@Service(DaXi.channelNo)
public class DaXi extends AbstractPay {

    static final String channelNo = "daxi";

    private static final String payUrl = "https://pay.91udd.com/Pay_Index.html";

    private final Map<String, String> payTypeMap = new HashMap<>();

    public DaXi() {
        payTypeMap.put(OutChannel.alih5.name(), "928");//MG企业支付宝扫码
        payTypeMap.put(OutChannel.alipay.name(), "919");//MY支付宝扫码（企业码）
        payTypeMap.put(OutChannel.aliwap.name(), "921");//支付宝wap小额话费
        payTypeMap.put(OutChannel.wechatwap.name(), "920");//微信wap小额话费
        payTypeMap.put(OutChannel.wechatpay.name(), "902");//微信扫码
        payTypeMap.put(OutChannel.onlinepay.name(), "930");//EMC云闪付
    }
    ...
    public OrderApiRespParams order(ChannelEntity channel, MerchantEntity merchant, McpConfigEntity mcpConfig, OrderReqParams reqParams) {
        LogByMDC.info(channelNo, "大溪支付请求：{}", JSON.toJSONString(reqParams));
    ...
    private Map<String, String> getParamsMap(McpConfigEntity mcpConfig, OrderReqParams reqParams) {
        ...
        Assert.notNull(payType, "大溪不支持的支付方式:" + reqParams.getOutChannel());
    ...
    private String getSignStr(Map<String, String> params, String upPublicKey) {
        ...
        LogByMDC.info(channelNo, "大溪支付支付加签参数：{}", signatureContent);
    ...
    public String callback(ChannelEntity channel, MerchantEntity merchant, McpConfigEntity mcpConfig, OrderEntity order, Map<String, String> params) {
        LogByMDC.info(channelNo, "大溪支付回调内容：{}", params);
        if (order.getOrderStatus().equals(OrderStatus.succ)) {
            LogByMDC.error(channelNo, "大溪支付回调订单：{}，重复回调", order.getOrderNo());
            return "ok";
        }
        ...
        if (!signVerify) {
            LogByMDC.error(channelNo, "大溪支付回调订单：{}，回调验签失败", order.getOrderNo());
            return "fail";
        }
        ...
        if (!"00".equals(returnCode)) {
            LogByMDC.error(channelNo, "大溪支付回调订单：{}，支付未成功{}，不再向下通知", trade_no);
            return "ok";
        }
        ...
        try {
            ...
            LogByMDC.info(channelNo, "大溪支付回调订单：{}，下发通知成功", order.getOrderNo());
        } catch (Exception e) {
            LogByMDC.error(channelNo, "大溪支付回调订单：{}，下发通知失败{}", order.getOrderNo(), e.getMessage());
            throw new RException("大溪支付下发通知报错:" + e.getMessage());
        }
        System.out.println("大溪支付下发通知成功..................................");
        return "ok";
    }
    ...
    private boolean verifySignParams(Map<String, String> params, String upMerchantKey) {
        ...
        LogByMDC.info(channelNo, "大溪支付回调订单:{}，回调参与验签参数:{}", params.get("orderid"), signParams);
