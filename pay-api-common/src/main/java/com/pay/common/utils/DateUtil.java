...
public class DateUtil {
    ...
    public final static String DATE_MOUNTH = "yyyy-MM";

    public final static String DATE_HOURS = "HH:mm";

    public final static String DATE_TIME_PATTERN = "yyyy-MM-dd HH:mm:ss";

    public final static String TIME_PATTERN = "HHmmss";

    public static long differ(String date1, String date2) {
        try {
            Date parse2 = shortSdf.parse(date2);
            Date parse1 = shortSdf.parse(date1);
            long diff = parse2.getTime() - parse1.getTime();
            return Math.abs(diff / (24 * 60 * 60 * 1000));
        } catch (Exception e) {
            throw new CustomerException("日期相减计算异常！");
        }
    }

    public static String numberToZh(int intInput) {
        ...
        } else if (si.length() == 2)// 十
        {
            ...
            sd += numberToZh(intInput % 10);
        } else if (si.length() == 3)// 百
        {
            sd += (GetCH(intInput / 100) + "百");
            if (String.valueOf(intInput % 100).length() < 2)
                sd += "零";
            sd += numberToZh(intInput % 100);
        } else if (si.length() == 4)// 千
        {
            sd += (GetCH(intInput / 1000) + "千");
            if (String.valueOf(intInput % 1000).length() < 3)
                sd += "零";
            sd += numberToZh(intInput % 1000);
        } else if (si.length() == 5)// 萬
        {
            sd += (GetCH(intInput / 10000) + "萬");
            if (String.valueOf(intInput % 10000).length() < 4)
                sd += "零";
            sd += numberToZh(intInput % 10000);
        }
     ...
     * @return 时间字符串
     */
    public static String toStr02(String format, Date time) {
        SimpleDateFormat df = null;
        if (null == format) {
    ...
    public static String toStr03(String format, Date time) {
        SimpleDateFormat df = null;
        if (null == format) {
    ...
    public static Date toDate(String source) {
        ...
        try {
            ...
            throw new CustomerException("字符串轉時間Date異常");
        }
    }
    ...
    public static List<String> findDates(Date dBegin, Date dEnd) {
        ...
        return lDate.stream().map(e -> toStr(DATE_PATTERN, e)).collect(toList());
    }
     ...
     * @return
     */
    public static String getFutureDate(Date date, int past) {
        Calendar c = Calendar.getInstance();
        c.setTime(date);
        c.add(Calendar.DAY_OF_MONTH, past);// 今天+1天
        date = c.getTime();
    ...
    private static void test() {
        ...
        System.out.println("当前旬开始：" + toStr(null, getTenDayStartTime(date)));
        System.out.println("当前旬结束：" + toStr(null, getTenDayEndTime(date)));
